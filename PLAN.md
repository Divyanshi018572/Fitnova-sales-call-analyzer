...
- [ ] **4:00–8:00** — `feature/transcription`: Whisper wrapper, dual-channel split. Test: one real
...
- [ ] **26:00–28:00** — `feature/edge-cases`: implement §5 table, add/adjust tests for each row.
      Merge.
- [ ] **28:00–29:00** — `feature/deploy`: `render.yaml` Blueprint (db + api + dashboard), push,
...
- [ ] **29:00–30:00** — README.md (setup — both `docker-compose up --build` locally and the
      deployed link, what is real versus mocked), writeup (A/B/C sections from the PDF, in your own
      words, concise).
- [ ] **30:00–31:00** — Buffer + record 2-minute video (trade-offs, what you didn't build and why,
